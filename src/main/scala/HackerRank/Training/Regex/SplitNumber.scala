...
/**
  ...
  */
private[this] object SplitNumber {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val phoneNumbers = next[String, Vector](nextLine(), n)
    val regex = "(\\d{1,3})(?:-| )(\\d{1,3})(?:-| )(\\d{4,10})".r
    phoneNumbers
      .map { phoneNumber =>
        val `match` = regex.findAllIn(phoneNumber)
        s"CountryCode=${`match`.group(1)},LocalAreaCode=${`match`.group(2)},Number=${`match`.group(3)}"
      }
      .foreach(println)
  }
