...
/**
  ...
  */
private[this] object SansaAndXor {
  ...
  private[this] def solve(): Unit = {
    ...
    for (_ <- 1 to t) {
      val n = next[Int]()
      println(next[Int, Vector](n).xorAllSubsequences)
    }
  }

  private[this] implicit final class SeqExtension(xs: Seq[Int]) {
    def xorAllSubsequences: Int = {
      if (xs.size % 2 == 0) {
        0
      } else {
        xs.grouped(2).map(_.head).reduce(_ ^ _)
      }
    }
  }
