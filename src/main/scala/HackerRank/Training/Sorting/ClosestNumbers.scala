...
*/
private[this] object ClosestNumbers {
  ...
  private[this] def solve(): Unit = {
    ...
    println(
      nextInt[Vector](n)
        .sorted
        .sliding(2)
        .foldLeft(Vector((0, 0, Int.MaxValue))) { case (results, pair) =>
          val minDistance = results.head._3
          val distance = Math.abs(pair.head - pair.last)
          if (distance < minDistance) {
            Vector((pair.head, pair.last, distance))
          } else if (distance == minDistance) {
            results :+ (pair.head, pair.last, distance)
          } else {
            results
          }
        }
