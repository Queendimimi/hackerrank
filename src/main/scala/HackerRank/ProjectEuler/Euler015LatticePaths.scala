...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 8/19/2017
  */
private[this] object Euler015LatticePaths {
  ...
  private[this] def solve(): Unit = {
    val t = next[Int]()
    val grids = next[Grid, Vector](Grid(next[Int](), next[Int]()), t)
    grids
      .map(grid => choose(grid.n + grid.m, grid.m) % BigInt(1000000007))
      .foreach(println)
  }

  private[this] final case class Grid(n: Int, m: Int)

  private[this] type ==>[I, O] = Memo[I, I, O]

  private[this] lazy val choose: (Int, Int) ==> BigInt = Memo {
    case (n, 0) =>
      require(0 <= n)
      1
    case (n, k) if k == n =>
  ...
  private[this] case class Memo[I, K, O](f: I => O)(implicit ev$1: I => K) extends (I => O) {
    type Input = I
    type Key = K
    type Output = O
