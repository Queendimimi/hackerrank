...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 10/28/2017
  */
private[this] object IceCreamParlor {
  ...
  private[this] def solve(): Unit = {
    val t = next[Int]()
    next[Trip, Vector](Trip(next[Int](), next[Int, Vector](next[Int]())), t)
      .map(_.find2BestFlavors())
      .map(_.mkString(" "))
      .foreach(println)
  }

  private[this] final case class Trip(budget: Int, flavorCosts: Seq[Int]) {
    def find2BestFlavors(): Vector[Int] = {
      val sortedFlavorCosts = flavorCosts.zipWithIndex.sorted

      var cost = -1
      var left = 0
      var right = flavorCosts.size - 1
      while (cost != budget && left != flavorCosts.size - 1 && right != 0) {
        cost = sortedFlavorCosts(left)._1 + sortedFlavorCosts(right)._1
        if (cost > budget) {
          right -= 1
        }
        if (cost < budget) {
