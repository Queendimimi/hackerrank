...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 8/12/2017
*/
private[this] object UkAndUs {
  ...
  private[this] def solve(): Unit = {
    ...
    val t = next[Int]()
    val americanWords = next[String]().split("\n")

    americanWords
      .map { americanWord =>
        val englishAndAmerican = s"(\\b$americanWord\\b|${getEnglish(americanWord)}\\b)".r
        englishAndAmerican.findAllIn(text).size
      }
      .foreach(println)
  }

  private[this] def getEnglish(americanWord: String) = {
    americanWord.dropRight(2) + "se"
  }
