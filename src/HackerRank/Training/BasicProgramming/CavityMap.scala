...
object CavityMap {
  ...
  private def solve(): Unit = {
    val n = nextInt()
    val grid = next[Array[Char], Array](nextChar[Array](n), n)

    val cavities = Vector.newBuilder[(Int, Int)]
    for {
      i <- 1 until n - 1
      j <- 1 until n - 1
    } yield {
      ...
      val isCavity = center > up && center > down && center > left && center > right
      if (isCavity) {
        cavities += ((i, j))
      }
    }
    cavities.result().foreach { case (i, j) => grid(i)(j) = 'X' }

    out.println(grid.map(_.mkString("")).mkString("\n")
    )
  }
