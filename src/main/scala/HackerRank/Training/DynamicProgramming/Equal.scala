...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 8/9/2017
*/
private[this] object Equal {
  ...
  private[this] def solve(): Unit = {
    ...
    for (_ <- 0 until t) {
      val n = next[Int]()
      println(next[Int, Vector](n).minimalNumberOfOperation())
    }
  }

  private[this] implicit final class SeqImprovements[T: Integral](underlying: Seq[T]) {
    import Integral.Implicits._
    import Ordering.Implicits._

    require(underlying.nonEmpty)

    private[this] val five = implicitly[Integral[T]].fromInt(5)
    ...
    def minimalNumberOfOperation():T = {

      val min = underlying.min

      def helper(min: T): T = {
        var count = zero

        for (current <- underlying) {
          val temp = current - min
          count += temp / five
          count += temp % five / two
          count += temp % five % two
        }
      ...
      var i = one
      while (i < five) {
        result = Vector(result, helper(min - i)).min
        i += one
      }
      result
    }
