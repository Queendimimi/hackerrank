...
/**
  ...
  */
private[this] object LargestRectangle {
  ...
  private[this] def solve(): Unit = {
    val n = nextInt()
    println(Histogram(next[Long, Vector](n)).maxArea)
  }

  private[this] class Histogram[@specialized(Int, Long, Double) T: Numeric](underlying: Seq[(T, T)]) {
    def maxArea: T = {
      import Numeric.Implicits._
      import Ordering.Implicits._

      val histogramWithWidthOne = underlying.map { case (height, width) => height * width }
      ...
      def area(right: Int, stack: mutable.ArrayStack[Int] = stack) = {
        ...
        val area = smallestBar * implicitly[Numeric[T]].fromInt(boxWidth)
      ...
      @tailrec
      def iterate(i: Int = 0, j: Int = 0, stack: mutable.ArrayStack[Int] = stack): Int = {
        if (i == histogramWithWidthOne.size) {
          i
        } else {
          if (stack.isEmpty || histogramWithWidthOne(stack.head) <= histogramWithWidthOne(i)) {
            stack.push(i)
            iterate(i + 1, j + 1)
          } else {
            area(i, stack)
  ...
  private[this] object Histogram {
    def apply[T: Numeric](underlying: Seq[T]): Histogram[T] = {
      new Histogram(underlying.zip(Stream.continually(implicitly[Numeric[T]].one)))
    }
  }
