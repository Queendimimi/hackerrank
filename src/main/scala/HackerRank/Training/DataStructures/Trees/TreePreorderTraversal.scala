...
/**
  ...
  */
private[this] object TreePreorderTraversal {
  ...
  // BinaryTree
  //------------------------------------------------------------------------------------------//
  private[this] sealed trait BinaryTree[+T] {

    @tailrec
    private[this] def foldLoop[A, B](accumulator: List[BinaryTree[A]], zeroValue: B)
                                    (f: (B, A) => B)
                                    (combine: (Branch[A], List[BinaryTree[A]]) => List[BinaryTree[A]]): B = {
      accumulator match {
        case (b: Branch[A]) :: tail => foldLoop(combine(b, tail), zeroValue)(f)(combine)
        case (l: Leaf[A]) :: tail => foldLoop(tail, f(zeroValue, l.value))(f)(combine)
        case (e: Eval[A]) :: tail => foldLoop(tail, f(zeroValue, e.value))(f)(combine)
        case Empty :: tail => foldLoop(tail, zeroValue)(f)(combine)
        case _ => zeroValue
      }
    }

    def foldPreOrder[B](zeroValue: B)(f: (B, T) => B): B = {
      foldLoop(List(this), zeroValue)(f) {
        (branch, tail) => Eval(branch.value) :: branch.left :: branch.right :: tail
      }
    }

    def foldInOrder[B](zeroValue: B)(f: (B, T) => B): B = {
      foldLoop(List(this), zeroValue)(f) {
        (branch, tail) => branch.left :: Eval(branch.value) :: branch.right :: tail
      }
    }

    def foldPostOrder[B](zeroValue: B)(f: (B, T) => B): B = {
      foldLoop(List(this), zeroValue)(f) {
        (branch, tail) => branch.left :: branch.right :: Eval(branch.value) :: tail
      }
    }

    def foldLevelOrder[B](zeroValue: B)(f: (B, T) => B): B = {
      foldLoop(List(this), zeroValue)(f) {
        (branch, tail) => (Eval(branch.value) :: tail) ::: List(branch.left, branch.right)
      }
    }

    private[this] case class Eval[A](value: A) extends BinaryTree[A]

  }

  private[this] final case object Empty extends BinaryTree[Nothing]

  private[this] final case class Leaf[A](value: A) extends BinaryTree[A]

  private[this] final case class Branch[A](value: A,
                                           left: BinaryTree[A],
                                           right: BinaryTree[A])
