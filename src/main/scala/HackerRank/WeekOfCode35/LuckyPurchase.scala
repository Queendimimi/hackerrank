...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 11/13/2017
*/
private[this] object LuckyPurchase {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val result = next[(String, Int), Vector]((next[String](), next[Int]()), n)
      .filter { case (name, price) =>
        val priceString = price.toString
        val fours = priceString.count(_ == '4')
        val sevens = priceString.count(_ == '7')
        fours + sevens == priceString.length && fours != 0 && fours == sevens
      }
      .reduceOption { (a, b) => if (a._2 > b._2) b else a }
      .fold("-1")(_._1)
