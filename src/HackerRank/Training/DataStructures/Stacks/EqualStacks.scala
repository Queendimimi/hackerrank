...
/**
  ...
  */
private[this] object EqualStacks {
  ...
  private[this] def solve(): Unit = {
    val (n1, n2, n3) = (nextInt(), nextInt(), nextInt())
    val a = nextInt[Vector](n1)
    val b = nextInt[Vector](n2)
    val c = nextInt[Vector](n3)
    println(makeEqualStacks(Vector(a, b, c)).head.sum)
  }

  private[this] def makeEqualStacks[T: Numeric](stacks: Seq[Seq[T]]): Seq[Seq[T]] = {
    import Numeric.Implicits._

    final case class Stack(value: mutable.ArrayStack[T], sum: T)

    @tailrec
    def makeEqual(stacks: Array[(Stack, Int)]): Array[(Stack, Int)] = {
      if (haveEqualSum(stacks)) {
        stacks
      } else {
        val maxStack = stacks.maxBy(_._1.sum)
        val head = maxStack._1.value.pop()
        stacks(maxStack._2) = (maxStack._1.copy(sum = maxStack._1.sum - head), maxStack._2)
        makeEqual(stacks)
      }
    }

    def haveEqualSum(stacks: Seq[(Stack, Int)]) = {
      !stacks
        .sliding(2)
        .exists(x => x.head._1.sum != x.last._1.sum)
    }

    makeEqual(
      stacks
        .map(x => Stack(x.to[mutable.ArrayStack], x.sum)).zipWithIndex.toArray
    ).map(_._1.value)
  }
