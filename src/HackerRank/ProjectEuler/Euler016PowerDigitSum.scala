...
/**
  ...
  */
private[this] object Euler016PowerDigitSum {
  ...
  private[this] def solve(): Unit = {
    val t = nextInt()
    nextInt[Vector](t).foreach(x => println(powerDigitSum(x)))
  }

  private[this] def powerDigitSum(n: Int) = {
    BigInt(2).pow(n).toString().foldLeft(BigInt(0))(_ + _.asDigit)
  }
