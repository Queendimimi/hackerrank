...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 6/9/2017
*/
private[this] object HappyLadybugs {
  ...
  private[this] def solve(): Unit = {
    val g = nextInt()
    val input = next[(Int, String), Vector]((nextInt(), nextString()), g)

    input
      .map { case (_, string) =>
        val _count = string.count(_ == '_')
        val charCount = string
          .groupBy(identity)
          .filter(_._1 != '_')
          .mapValues(_.length)

        lazy val alreadyHappy = _count == 0 && string
          .split("(?<=(.))(?!\\1)")
          .map(s => (s.head, s.length))
          .filter(_._2 > 1)
          .toMap == charCount

        val canAllBeHappy = _count > 0 && charCount
          .forall { case (_, count) => count > 1 }
        if (canAllBeHappy || alreadyHappy) "YES" else "NO"
      }
      .foreach(println)
  }
