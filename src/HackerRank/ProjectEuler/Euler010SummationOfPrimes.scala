...
private[this] object Euler010SummationOfPrimes {
  ...
  private[this] def solve(): Unit = {
    val t = nextInt()
    val limits = nextInt[Vector](t)
    val sumPrimesUpTo = new PrimeSummer(limits.max)

    limits.foreach(x => println(sumPrimesUpTo(x)))
  }


  private[this] final class PrimeSummer(ub: Int) extends (Int => Long) {
    private[this] val primes = sieveOfEratosthenes(ub)
    private[this] var sum = 0L

    private[this] val sumCache = Array.fill[Long](ub + 1)(0L)
    for (i <- 2 to ub) {
      if (primes(i)) sumCache(i) = {
        sum += i
        sum
      } else {
        sumCache(i) = sum
    ...
    def apply(ub: Int): Long = {
      sumCache(ub)
    }

    private[this] def sieveOfEratosthenes(ub: Int) = {
      val (sieve, sqrtLimit) = (Array.fill[Boolean](ub + 1)(true), math.sqrt(ub).toInt)
      ...
      for (i <- 2 to sqrtLimit) {
        if (sieve(i)) {
          for (j <- i * i to ub by i) sieve(j) = false
        }
      }
      sieve.toVector
    }
