...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 5/13/2017
  */
private[this] object JumpingOnTheCloudsRevisited {
  ...
  private[this] def solve(): Unit = {
    val n = nextInt()
    val k = nextInt()
    val isThunderCloud = nextInt[Array](n).map(x => if (x == 0) false else true)
    println(100 - (0 until n by k).map(i => if (isThunderCloud(i)) 3 else 1).sum)
  }
