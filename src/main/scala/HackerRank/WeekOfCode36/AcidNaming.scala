...
/**
  ...
  */
private[this] object AcidNaming {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val acids = next[String, Vector](n)
    acids.map(nameAcid).foreach(println)
  }

  private[this] def nameAcid(name: String): String = {
    if (name.startsWith("hydro") && name.endsWith("ic")) {
      "non-metal acid"
