...
/**
  ...
  */
private[this] object FindHackerrank {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val lines = next[String, Vector](nextLine(), n)
    lines
      .map(s => findMention(s).fold(-1) {
        case Start => 1
        case End => 2
        case StartAndEnd => 0
      })
  ...
  private[this] sealed trait HackerRankMention

  private[this] final case object Start extends HackerRankMention

  private[this] final case object End extends HackerRankMention

  private[this] final case object StartAndEnd extends HackerRankMention

  private[this] def findMention(line: String): Option[HackerRankMention] = {
    def search(r: Regex): Boolean = {
      r.findAllIn(line).matchData.map(_.group(0))
        .collect { case address if address == line => true }
        .hasNext
    }

    val foundAtStart = search("(^hackerrank .*$)|(^hackerrank$)".r)
    val foundAtEnd = search("(^.* hackerrank$)|(^hackerrank$)".r)

    if (foundAtStart & foundAtEnd) {
      Option(StartAndEnd)
