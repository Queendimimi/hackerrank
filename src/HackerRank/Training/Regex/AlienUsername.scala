...
/**
  ...
  */
private[this] object AlienUsername {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val userNames = next[String, Vector](n)

    userNames
      .map { userName =>
        val regex = "(_|\\.)[0-9]+[a-zA-Z]*(_|)".r
        if (regex.findAllIn(userName).foldLeft("")(_ + _) == userName) "VALID" else "INVALID"
      }
      .foreach(println)
  }
