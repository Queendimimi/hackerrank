...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 8/13/2017
*/
private[this] object DetectHtmlTags {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val matchStartTag = "<\\b(\\w+)\\b.*?>".r
    val text = readAll()
    val builder = SortedSet.newBuilder[String]

    builder ++= matchStartTag.findAllIn(text).matchData.map(_.group(1))

    println(builder.result().mkString(";"))
  }
