...
private[this] object CountingValleys {
  ...
  private[this] def solve(): Unit = {
    val n = nextInt()
    val walk = nextChar[Vector](n)
    val result = walk.map {
      case 'U' => 1
      case 'D' => -1
    }.foldLeft((0, 0, 0)) { case ((valleyCount, prevHeight, height), step) =>
      if (height == -1 && prevHeight == 0) {
        (valleyCount + 1, height, height + step)
      } else {
        (valleyCount, height, height + step)
      }
    }._1
