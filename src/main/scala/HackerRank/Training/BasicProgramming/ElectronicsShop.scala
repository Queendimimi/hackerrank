...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 5/21/2017
*/
private[this] object ElectronicsShop {
  ...
  private[this] def solve(): Unit = {
    ...
    val n = nextInt()
    val m = nextInt()
    val keyboards = nextInt[Vector](n).sortBy(-_)
    val usbSticks = nextInt[Vector](m).sorted

    val result = keyboards.flatMap { keyboard =>
      usbSticks.takeWhile(keyboard + _ <= s).map(keyboard + _)
