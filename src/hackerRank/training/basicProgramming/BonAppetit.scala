...
object BonAppetit {
  ...
  private def solve(): Unit = {
    val n = nextInt()
    val k = nextInt()
    val itemCosts = nextInt[Array](n)
    val charged = nextInt()

    val cost = itemCosts.sum
    val splitCost = cost - itemCosts(k)
    val actual = splitCost / 2
    val difference = charged - actual
    if (difference != 0) out.println(difference) else out.println("Bon Appetit")
  }
