...
*/
private[this] object TwosComplement {
  ...
  private[this] def solve(): Unit = {
    val t = next[Int]()
    next[(Int, Int), Vector]((next[Int](), next[Int]()), t).foreach { x =>
      val (from, to) = x
      println(bitCount(from, to))
    }
  }

  private[this] def bitCount(from: Int, to: Int): Long = {
    require(from <= to)
    if (from <= 0 && to >= 0) {
      bitCountInRangeToZero(from) + bitCountInRangeToZero(to)
    } else if (from < 0 && to < 0) {
      bitCountInRangeToZero(from) - bitCountInRangeToZero(to + 1)
    } else {
      bitCountInRangeToZero(to) - bitCountInRangeToZero(from - 1)
    }
  }

  private[this] def bitCountInRangeToZero(x: Int): Long = {
    def bitCountFromZeroTo(x: Long): Long = {
      val bitCount: (Long) => Long = java.lang.Long.bitCount

      if (x == 0) {
        0
      } else if (x % 2 == 0) {
        bitCountFromZeroTo(x - 1) + bitCount(x)
      } else {
        (x.toLong + 1) / 2 + 2 * bitCountFromZeroTo(x / 2)
      }
    }

    if (x < 0) 32 * (-x.toLong) - bitCountFromZeroTo(-x.toLong - 1) else bitCountFromZeroTo(x)
  }
