...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 8/14/2017
  */
private[this] object FindSubstring {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val text = next[String, Vector](nextLine(), n).mkString(" ")
    val q = next[Int]()
    val queries = next[String, Vector](nextLine(), q)
    queries.foreach(query => println(countSubWords(text, query)))
  }

  private[this] def countSubWords(text: String, query: String) = {
    val wordMatcher = "\\w+".r
    val queryMatcher = ("(?<=\\w)" + query + "(?=\\w)").r
    wordMatcher.findAllIn(text).matchData.map(_.group(0))
      .map(word => queryMatcher.findAllIn(word).size)
      .sum
  }
