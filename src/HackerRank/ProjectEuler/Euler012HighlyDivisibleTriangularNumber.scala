...
/**
  ...
  */
private[this] object Euler012HighlyDivisibleTriangularNumber {
  ...
  private[this] def solve(): Unit = {
    val t = next[Int]()
    next[Int, Vector](t)
      .foreach(x => println(x.findFirstTriangularNumberWithMoreDivisors))
  }

  private[this] implicit def intToTriangleNumber(limit: Int): TriangleNumber[Int] = {
    new TriangleNumber(limit)
  }

  //inorder to pass you have to replace all generics with ints
  private[this] final class TriangleNumber[@specialized(Int) T: Integral](limit: T) {
    private[this] val three = implicitly[Integral[T]].fromInt(3)
    private[this] val two = implicitly[Integral[T]].fromInt(2)
    private[this] val one = implicitly[Integral[T]].one
    private[this] val zero = implicitly[Integral[T]].zero

    def findFirstTriangularNumberWithMoreDivisors: T = {
      import Integral.Implicits._
      import Ordering.Implicits._

      var triangularNumberIndex = one
      var lNum = countDivisors(triangularNumberIndex)
      var rNum = countDivisors(triangularNumberIndex + one)

      while (lNum * rNum <= limit) {
        triangularNumberIndex += one
        lNum = rNum
        rNum = countDivisors(triangularNumberIndex + one)
      }
      sumTo(triangularNumberIndex)
    }


    private[this] def sumTo(n: T): T = {
      import Integral.Implicits._

      (n * (n + one)) / two
    }

    private[this] def countDivisors(x: T): T = {
      import Integral.Implicits._

      var n = if (x % two == zero) x / two else x
      var divisorCount = one
      var count = zero
      ...
      divisorCount = divisorCount * (count + one)
      var p = three
      while (n != one) {
        count = zero
        while (n % p == zero) {
