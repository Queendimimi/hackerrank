...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 8/22/2017
  */
private[this] object SayingHi {
  ...
  private[this] def solve(): Unit = {
    ...
    next[String, Vector](nextLine(), n)
      .map("(^(H|h)(I|i)\\s(?!D|d).*$)".r.unapplySeq)
      .collect { case Some(head :: tail) => head }
      .foreach(println)
  }
