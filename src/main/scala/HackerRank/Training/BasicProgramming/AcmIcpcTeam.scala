...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 8/11/2017
  */
private[this] object AcmIcpcTeam {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val m = next[Int]()
    val persons = next[BitSetWithRefEquality, List](
      BitSetWithCustomEquals(
        next[Char, Vector](m)
          .zipWithIndex
          .collect { case (c, index) if c == '1' => index }: _*
      ), n)

    val bestTeamsCount = persons.countBestTeamCombinations()

    println(bestTeamsCount._1)
    println(bestTeamsCount._2)
  }

  private[this] implicit final class SeqImprovements(val underlying: Seq[BitSetWithRefEquality])
    extends AnyVal {
    def countBestTeamCombinations(): (Int, Int) = {
      underlying.combinations(2)
        .foldLeft(0, 0) { case ((maxTopics, numberOfTeams), team) =>
          val coveredTopics = team.reduce(_ | _).size
          if (maxTopics < coveredTopics) {
            (coveredTopics, 1)
          } else if (maxTopics == coveredTopics) {
            (maxTopics, numberOfTeams + 1)
          } else {
            (maxTopics, numberOfTeams)
          }
        }
    }
  }

  private[this] implicit def bRef2b(underlying: BitSetWithRefEquality): BitSet = {
    underlying.underlying
  }

  private[this] implicit def b2bRef(underlying: BitSet): BitSetWithRefEquality = {
    new BitSetWithRefEquality(underlying)
  }

  private[this] final class BitSetWithRefEquality(val underlying: BitSet) {
  ...
  private[this] object BitSetWithCustomEquals {
    def apply(elems: Int*): BitSetWithRefEquality = new BitSetWithRefEquality(BitSet(elems: _*))
  }
