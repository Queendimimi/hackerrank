...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 10/31/2017
*/
private[this] object MinimumLoss {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val costs = next[Long, Vector](n)

    val minimumLoss = costs
      .zipWithIndex
      .sortBy(_._1)
      .sliding(2)
      .map { vector =>
        val first = vector.head
        val second = vector(1)
        (second._1 - first._1, first._2 - second._2)
      }
      .filter(_._2 > 0)
      .minBy(_._1)._1
