...
object SockMerchant {
  ...
  private def solve(): Unit = {
    val n = nextInt()
    val socks = nextIntAsMap(n)
    val sockPairs = socks.values.map(_ / 2).sum
    out.println(sockPairs)
  }

  private def nextIntAsMap(n: Int) = {
    val builder = mutable.Map.empty[Int, Int]
    for (_ <- 0 until n) {
      val current = nextInt()
      builder.get(current) match {
        case Some(count) => builder += current -> (count + 1)
        case None => builder += current -> 1
      }
    }
    builder
  }
