...
/**
  ...
  */
private[this] object NonDivisibleSubset {
  ...
  private[this] def solve(): Unit = {
    ...
    println(next[Int, Set](n).numberOfNonDivisibleSubsets(k))
  }

  private[this] implicit final class SetImprovements[T: Integral](underlying: Set[T]) {
    ...
    //size of a maximal subset, of the underlying set where the sum of any 2 numbers in  is not evenly
    // divisible by k.
    def numberOfNonDivisibleSubsets(k: T): Int = {

      val counts = underlying
        .groupBy(_ % k)
        .mapValues(_.size)

      var total = Math.min(counts.getOrElse(zero, 0), 1)

      if (k % two == 0) {
        for (i <- NumericRange(one, k / two, one)) {
          total += Math.max(counts.getOrElse(i, 0), counts.getOrElse(k - i, 0))
        }
        total += 1
      } else {
        for (i <- NumericRange.inclusive(one, k / two, one)) {
          total += Math.max(counts.getOrElse(i, 0), counts.getOrElse(k - i, 0))
        }
      }
