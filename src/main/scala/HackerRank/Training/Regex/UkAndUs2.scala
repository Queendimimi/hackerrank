...
private[this] object UkAndUs2 {
  ...
  private[this] def solve(): Unit = {
    ...
    val t = next[Int]()
    val englishWords = next[String]().split("\n")

    englishWords
      .map { englishWord =>
        val englishAndAmerican = s"(\\b$englishWord\\b|${getAmerican(englishWord)}\\b)".r
        englishAndAmerican.findAllIn(text).size
      }
      .foreach(println)
  }

  private[this] def getAmerican(englishWord: String) = {
    if (englishWord == "our") {
      englishWord
    } else {
      englishWord.replace("our", "or")
    }
  }
