...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 10/21/2017
  */
private[this] object HackerlandRadioTransmitters {

  import Reader._
  import Writer._

  private[this] val TEST_INPUT: Option[String] = Some("7 2\n9 5 4 2 6 15 12")
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    val k = next[Int]()
    println(minTransmitters(next[Int, Vector](n), k))
  }

  private[this] def minTransmitters(s: Seq[Int], transmitterRange: Int): Int = {
    val seq = s.sorted
    ...
    while (i < seq.length) {
      // left
      maxRange = seq(i) + transmitterRange
      while (i < seq.length && seq(i) <= maxRange) {
        i += 1
      }
