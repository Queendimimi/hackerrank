...
*/
private[this] object AppleAndOrange {
  ...
  private[this] def solve(): Unit = {
    ...
    val m = nextInt()
    val n = nextInt()
    val appleDistances = nextInt[Vector](m)
    val orangeDistances = nextInt[Vector](n)

    println(appleDistances.count(x => (x + a) >= s && (x + a) <= t))
    println(orangeDistances.count(x => (x + b) >= s && (x + b) <= t))
  }
