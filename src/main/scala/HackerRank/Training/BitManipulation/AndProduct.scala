...
/**
  ...
  *
  * @author A. Roberto Fischer <a.robertofischer@gmail.com> on 8/1/2017
*/
private[this] object AndProduct {
  ...
  private[this] def solve(): Unit = {
    val powersOf2 = powerIterator(2).take(32).to[Vector]
    val n = nextInt()

    next[(Long, Long), Vector]((nextLong(), nextLong()), n)
      .map { case (a, b) => a & powersOf2.map(_ + a).takeWhile(_ <= b).reduce(_ & _) & b }
      .foreach(println)

  }

  private[this] def powerIterator(i: Int): Iterator[Long] = {
    Iterator.iterate(1L)(_ * i)
  }
