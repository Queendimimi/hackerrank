...
private[this] object CutTheSticks {
  ...
  private[this] def solve(): Unit = {
    val n = next[Int]()
    println(next[Int, Vector](n).countCuts().mkString("\n"))
  }
  ...
  private[this] implicit final class SeqImprovements[T: Integral](underlying: Seq[T]) {
    ...
    def countCuts(): Seq[Int] = {
      ...
      def recursiveCuts(xs: Seq[T]): Unit = {
        if (xs.nonEmpty) {
          cuts += xs.size
          val toBeRemoved = xs.takeWhile(_ <= xs.head)
          recursiveCuts(xs.drop(toBeRemoved.size).map(_ - xs.head))
        }
      }

      recursiveCuts(underlying.sorted)
      cuts.result()
    }
  }
